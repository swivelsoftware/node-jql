...
export function createFunction(function_: string) {
  ...
  if (!function_.startsWith('function')) throw new JQLError(`invalid function '${function_}'`)

  const argsIndex = [function_.indexOf('(') + 1, function_.indexOf(')')]
  const bodyIndex = [function_.indexOf('{') + 1, function_.lastIndexOf('}')]
  if (argsIndex[1] > bodyIndex[0]) throw new JQLError(`invalid function '${function_}'`)
  if (bodyIndex[1] - bodyIndex[0] === 1) throw new JQLError(`empty function '${function_}'`)
